...
function Home() {
    ...
    const handleSubmitRideAlong = async (e) => {
        ...
        try {
            ...
            // const now = new Date();
            //
            // // Filter de resultaten om alleen de toekomstige ritten te behouden
            // const futureRides = result.data.filter(ride => new Date(ride.departureDate) >= now);
            //
            // // Nu heb je toekomstige ritten in de variabele futureRides. Hier kun je er verder mee.
            //
            // history.push(`/rides?pickUpLocation=${pickUpLocation}&destination=${destination}&pax=${pax}&departureDate=${departureDate}`);

            // originele hieronder 24/7
            history.push(`/rides?pickUpLocation=${pickUpLocation}&destination=${destination}&pax=${pax}&departureDate=${departureDate}`);
